...
Stories de haut niveau (implémentées sur `Proto_manaGeneral`) :
- **Chaque classe a 6 cartes propres, jamais partagées (2026-08-24, remplace les 2 cartes génériques communes)** : 3 "Départ" + 3 "Avancé", chacune incluant une copie de "Coup direct"/"Encaisser" propre à la classe (renommées "Main de feu"/"Barrière" pour le Mage — "Main de feu" a été renommée une 2ᵉ fois le 2026-08-24, ex-"Flamèche", et inflige désormais un vrai dégât de feu magique au lieu du dégât physique/mêlée d'origine, voir `gdd.md` → Card Types and Effects et Assumptions and Dependencies pour ce renversement de design).
...
**Statut :** en cours, cible Mois 2. Un premier squelette de run (budget de difficulté croissant, deck qui grossit par combat, persistance entre combats) est prototypé sous le nom **Run Infini** dans l'Epic 1 — volontairement sans carte de quêtes, sans récompense de fin de run ; à absorber/étendre par cet epic plutôt que dupliqué. **Premier vrai combat de boss implémenté et jouable (2026-08-21, ajusté 2026-08-24) :** voir story dédiée ci-dessous — la carte de quêtes, les biomes, la récompense de fin de run et le reste de la structure restent non démarrés.

Stories de haut niveau :
- **Boss "Homme Arbre" (implémenté 2026-08-21/24) :** combat fixe (1 Homme Arbre + 4 Pousses d'Arbre, jamais tiré par la génération aléatoire du Run Infini), accessible via le bouton "Tester le boss" au menu ou comme 6ᵉ combat d'un **run borné à 5 combats + 1 boss** (mode distinct du Run Infini par défaut). Homme Arbre : 3 coups ("Coup de Branche" — dégâts élevés sur un héros ciblé au hasard ; "Onde Sylvestre" — dégâts plus faibles à tous les héros vivants, nouvelle attaque ennemie en zone ; "Renaissance Sylvestre" — résurrection des Pousses d'Arbre déjà vaincues, jamais une invocation, disponible seulement si au moins une Pousse est morte). Pousse d'Arbre : PV bas, un seul coup faible. Rééquilibrage : PV/dégâts divisés par deux (trop fort en playtest), puis PV remontés à 80 une fois la sensibilité au feu ajoutée en contrepartie ; dégâts et PV des Pousses restent réduits. **Nouvelle mécanique — sensibilité au feu :** l'Homme Arbre subit +50% de dégâts de toute carte taguée "feu" (Main de feu, Tornade de feu, Boule de feu — voir `gdd.md` → Card Types and Effects), affiché par une ligne d'infobulle dédiée et un badge permanent dans son cadre. Sprites IA dédiés générés pour les 2 nouveaux ennemis. **Sortie de combat dédiée :** gagner ce combat précis saute le draft de carte ET le feu de camp (écran "Victoire ! / Le Boss est vaincu !" bref, puis retour direct au menu) — un combat normal continue de suivre le chemin habituel. Détail complet : voir `gdd.md` → "Boss de run borné — Homme Arbre".
