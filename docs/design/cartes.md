...
**Audit agent_doc du 2026-09-12** (relecture fraîche intégrale de `game/src/data/cards.lua`, 48 cartes, suite au redesign visuel des cartes qui a raccourci plusieurs textes et renommé 2 cartes) : les 2 renommages (« Combattant expérimenté »/« Coup direct » → **Combat aguerri**, code `coup-direct-guerrier` inchangé ; « Communion des morts » → **Lien morbide**, code `communion-morts` inchangé) étaient déjà correctement appliqués dans le tableau. 2 écarts trouvés et corrigés — les textes de **Riposte** et **Assassinat** (base ET amélioré pour les deux) portaient encore l'ancienne formulation longue, remplacée dans le code par une version raccourcie à mécanique strictement identique :
- Riposte : « Si "cibleennemi", annule TOUTES les attaques et inflige la moitié/totalité des dégâts en retour à chaque ennemi. » → « Annule chaque attaque de "cibleennemi", renvoie la moitié/totalité des dégâts. »
- Assassinat : « Si Camouflé, inflige 12/18 "epee", sinon gagne "Discrétion" 5/10, "Puissance" 2 et Assassinat va sur le dessus du deck. "Furtif" » → « Camouflé : 12/18 "epee". Sinon : "Discrétion" 5/10, "Puissance" 2, remonte sur le deck. "Furtif" »
...
- « Coup direct » renommée « Combattant expérimenté » (code interne inchangé, `coup-direct-guerrier`) et devient la première carte à utiliser le nouveau mode de ciblage `"enemy-or-ally"` (voir Notes de lecture ci-dessus) : le joueur choisit librement un ennemi (dégâts "epee") ou un allié (bouclier), jamais les deux à la fois sur le même lancer. Renommée une 2ᵉ fois « Combat aguerri » (2026-09-12, demande explicite -- nom trop long pour tenir sur 1 ligne dans le nouveau cartouche de carte), même code interne.
...
| Lien morbide | Avancé | 1 (+0-6 Corruption) | Support | sort, soin | Se soigne de 2×X. | Se soigne de 3×X. |
...
**Passe du 2026-09-02 (demande explicite, correction de comportement pur sans changement de texte affiché pour Rite mineur/Lien morbide) :**
- « Rite mineur »/« Lien morbide » (alors « Communion des morts », renommée le 2026-09-12 -- demande explicite, nom trop long pour tenir sur 1 ligne dans le nouveau cartouche de carte, code interne `communion-morts` inchangé) plafonnent désormais leur dépense de Corruption au soin RÉELLEMENT nécessaire pour revenir à PV max (`def.heal_per_corruption`, voir `Game.resolve_pending`) — elles ne vident plus systématiquement toute la Corruption disponible si le héros a moins besoin. « Servant d'os » partage le même mécanisme de plafond en Corruption (`corruption_cost_cap`) mais sans `heal_per_corruption` (dégâts, pas soin) : non concerné par ce changement, continue de dépenser tout le plafond disponible.
